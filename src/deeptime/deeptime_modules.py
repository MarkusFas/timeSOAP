...
def plot_dominant_component(feature_trajectory, ax, dxy, title):
    x, y = np.meshgrid(
    ...
    ax.scatter(*feature_trajectory.T[:2,:], marker='.')
...
def run_analysis(feature_trajectory, test_trajectory, lag, label):
    pca = PCA(n_components=1)
    pca.fit(feature_trajectory)
    pca_projection = pca.transform(test_trajectory)
    dxy_pca = pca.components_[0]  # dominant pca component

    vamp = VAMP(dim=1, lagtime=lag)
    vamp = vamp.fit(feature_trajectory).fetch_model()
    vamp_projection = vamp.transform(test_trajectory)
    dxy_vamp = vamp.singular_vectors_left[:, 0]  # dominant vamp component

    tica = TICA(dim=1, lagtime=lag)
    tica = tica.fit(feature_trajectory).fetch_model()
    tica_projection = tica.transform(test_trajectory)
    ...
    ax_projections.set_ylim(-3,3)
    ax_projections.plot(pca_projection, label='PCA')
    ax_projections.plot(tica_projection, label='TICA', alpha=.5)
    ax_projections.plot(vamp_projection, label='VAMP', linestyle='dashed', alpha=.5)
    ax_projections.legend()

    plot_dominant_component(feature_trajectory, ax_pca, dxy_pca, 'Samples with dominant PCA component')
    plot_dominant_component(feature_trajectory, ax_tica, dxy_tica, 'Samples with dominant TICA component')
    plot_dominant_component(feature_trajectory, ax_vamp, dxy_vamp, 'Samples with dominant VAMP component')
