...
def benchmark(CVmodel, system, eval_options, n_warmup=5, n_runs=20):
    """Time forward and backward pass separately.

    Returns
    -------
    fwd_times, bwd_times : lists of wall-clock seconds (length n_runs each)
    """
    fwd_times = []
    bwd_times = []

    for i in range(n_warmup + n_runs):
        # fresh grad tape each iteration
        pos = system.positions.detach().requires_grad_(True)
        register_autograd_neighbors(
            system,
            system.get_neighbor_list(CVmodel.requested_neighbor_lists()[0]),
        )

        _sync()
        t0 = _time.perf_counter()
        cv = CVmodel(systems=[system], options=eval_options, check_consistency=False)
        CV = cv["features"].block().values[0]
        _sync()
        t1 = _time.perf_counter()

        grad = torch.autograd.grad(CV, pos)[0]
        _sync()
...
def print_benchmark(label, times):
    arr = np.array(times) * 1e3  # → ms
    print(f"  {label:20s}  mean={arr.mean():.2f} ms  std={arr.std():.2f} ms"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--benchmark', action='store_true', help='Benchmark forward/backward timing on first frame and exit')
    parser.add_argument('--n-warmup', type=int, default=5, help='Warmup iterations for benchmark')
    parser.add_argument('--n-runs', type=int, default=20, help='Timed iterations for benchmark')
    ...
    structures = read(trj_name, index='::{}'.format(nskip))
    ...
    if args.benchmark:
        ...
        print(f"\n=== Benchmark: {model_name} | device={device} | "
              f"n_atoms={len(structures[0])} | warmup={args.n_warmup} | runs={args.n_runs} ===")
        fwd, bwd = benchmark(CVmodel, systems[0], eval_options,
                             n_warmup=args.n_warmup, n_runs=args.n_runs)
        print_benchmark("forward", fwd)
        print_benchmark("backward", bwd)
        print_benchmark("fwd+bwd", [f + b for f, b in zip(fwd, bwd)])
        exit()
